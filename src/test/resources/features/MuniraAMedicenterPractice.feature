...
Feature:Medicenter Test Scenarios
  ...
  Scenario Outline: Login to app data driven scenario
    ...
    Then I click on element with xpath "//button[contains(text(),'Login')]"
    And I wait for element with xpath "//a[contains(text(),'Back to home')]" to be present
    #use data driven scenarios for login page
  ...
  Scenario: Make an appointment and Delete an appointment
    ...
    Then I type "Unique appointment from MuniraA 1" into element with xpath "//textarea[@id='note']"
    Then I click on element with xpath "//select[@name='employee_id']"
    And I wait for 2 sec
    Then I click on element with xpath "//select[@name='employee_id']/..//option[2]"
    Then I type "05/12/2023" into element with xpath "//input[@id='date']"
    ...
    And I wait for element with xpath "//p[contains(text(),'Unique appointment from MuniraA 1')]" to be present
    #delete the appointment
    Then I wait for element with xpath "//p[contains(text(),'Unique appointment from MuniraA 1')]/ancestor::article/div[2]/span" to be present
    Then I click on element with xpath "//p[contains(text(),'Unique appointment from MuniraA 1')]/ancestor::article/div[2]/span"
    ...
    Then element with xpath "//p[contains(text(),'Unique appointment from MuniraA 1')]" should not be present
